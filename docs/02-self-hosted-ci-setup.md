...
从表面看，注册 runner 好像就是下载、解压、执行 GitHub 官方的 `config.sh`。我一开始也以为写一个几十行脚本就差不多了。

真正用起来以后才发现，难的不是“正常情况能跑”，而是各种边缘情况怎么处理。

比如 Linux 用户不一定叫 `actions`；有人会把 runner 放在自己的 home，也有人会指定 `./runners` 这种相对目录；同一个机器可能同时连接很多仓库；两个不同 owner 甚至可能有同名 repo；注册 token 可能已经过期；上一次失败可能只下载了一半文件；systemd service 可能已经存在，也可能被删了一半；删除 runner 时 GitHub 注销失败，但 systemd service 已经卸载成功。

这些情况单独看都不复杂，可一旦工具公开给别人使用，就不能假设每个人的机器和我自己完全一样。

所以现在的 `github-runner-tools` 做了不少看起来不那么显眼的保护。

比如它不会假设用户一定叫 `actions`，而是自动识别当前普通用户；默认把 runner 安装在这个用户真正的 home 目录；如果用户指定相对目录，会先转换成绝对路径，避免脚本中途 `cd` 以后路径失效。

它也不会再只用 repo 名命名 runner 目录，而是使用：

```text
owner--repo
```

如果名字太长，还会截断并加一个短 hash，确保路径和 runner name 不会无限增长。

脚本默认也不会使用 GitHub Runner 的 `--replace`。也就是说，如果 GitHub 上已经存在一个同名 runner，新注册会失败，而不是默默把旧 runner 替换掉。

对删除操作则更加保守。旧版本的 runner 目录可能只有 repo 名，没有 owner 信息，所以不能看到：

```text
actions-runner-project-a
```

就猜它一定属于某个 `OWNER/project-a`。现在删除脚本会读取 runner 自己保存的 `.runner` 元数据，确认 GitHub URL 真的是用户要求删除的那个仓库以后，才继续。

Systemd service 也是一样。确定不存在，可以继续；确定存在，就先卸载；如果状态无法判断，则停止。整个原则其实只有一句话：

**能够确认的事情自动做，不能确认的事情不要猜。**

这对于会实际执行代码、安装系统服务的工具，比“尽量什么都自动完成”更重要。
